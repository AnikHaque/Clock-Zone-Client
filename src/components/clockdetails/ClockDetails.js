...
const ClockDetails = () => {
   ...
   const onSubmit = data =>{
       console.log(data);
       axios.post("http://localhost:5000/clockbooking",data)
       .then(res=>{
    ...
    const [food,setFood] = useState({})
     useEffect(()=>{
         fetch(`http://localhost:5000/clock/${id}`)
       .then(res=>res.json())
...
<Container fluid>
  <Row>
    <Col className='shadow' sm={12} md={12} lg={6}><img src={food.img} className='img-fluid w-100 individual-image'></img></Col>
...
<br></br>
     ...
     <br></br>
<br></br>
     <input {...register("foodname", { required: true, })} placeholder="Clock Name" className='w-100 p-2' />

     <br></br>
<br></br>
     <input type="text" {...register("price")}placeholder="Clock Price" className='w-100 p-2' />
     <br></br>
<br></br>
<input {...register("image", { required: true, maxLength: 920 })} placeholder="Clock Image" className='w-100 p-2' />
<br></br>
<br></br>
